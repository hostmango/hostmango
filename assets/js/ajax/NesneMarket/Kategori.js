$(document).ready(function(){
  ...
  $(".Ekle").on("submit", function(e){
    ...
    $.ajax({
      type: "post",
      url: url+"/NesneMarket/KategoriEkle",
      contentType: false,
      processData: false,
      data: data,
      beforeSend: function(){
  ...
  $(document).on('click','[data-bs-target="#duzenle"]', function(event) {
    ...
    $.ajax({
      type: "get",
      url: url+"/NesneMarket/KategoriDetay/"+id,
      beforeSend: function(){
        $(".site_loading").show();
      },
      success: function(response){
        if (response.error) {
          ...
          $('.Duzenle [name="cat_main_id"]').html('<option value="'+response.cat_main_id['id']+'">'+response.cat_main_id['name']+'</option>');
          $('.Duzenle [name="cat_name"]').val(response.cat_name);
          $('.Duzenle [name="cat_id"]').val(response.cat_id);
        }
      },
      complete: function(){
        $(".site_loading").hide();
  ...
  $(".Duzenle").on("submit", function(e){
    ...
    $.ajax({
      type: "post",
      url: url+"/NesneMarket/KategoriDuzenle",
      contentType: false,
      processData: false,
      data: data,
      beforeSend: function(){
  ...
  $(document).on('click','.kategoriSil',function(e) {
    ...
    .then((result) => {
      if (result.value) {
        ...
        $.ajax({
          type: "post",
          url: url+"/NesneMarket/KategoriSil",
          data: {cat_id},
          success: function(response){
            if (response.error) {
              toastr["error"](response.error);
  ...
  $('.Kategoriler').select2({
    ajax: {
      url: url+'/NesneMarket/UstKategoriler',
      dataType: 'json',
      delay : 250,
